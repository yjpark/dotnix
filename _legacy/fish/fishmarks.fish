...
if not set -q NO_FISHMARKS_COMPAT_ALIASES
    alias gg go_to_bookmark
end

function bookmark_save --description "Save the current directory as a bookmark"
    set -l bn $argv[1]
    if [ (count $argv) -lt 1 ]
...
function bookmark_print --description "Print the directory associated with a bookmark"
    if [ (count $argv) -lt 1 ]
...
function bookmark_delete --description "Delete a bookmark"
    if [ (count $argv) -lt 1 ]
        echo -e "\033[0;31mERROR: bookmark name required\033[00m"
        return 1
    end
...
function bookmark_list --description "List all available bookmarks"
    if not _check_help $argv[1];
        cat $SDIRS | grep "^export DIR_" | sed "s/^export /set -x /" | sed "s/=/ /" | .
